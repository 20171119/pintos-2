...
//  msg ("tid of main thread %d\n ", thread_current()->tid); //

  lock_init (&lock);
  lock_acquire (&lock);
//  msg("main got lock\n");
  thread_create ("acquire", PRI_DEFAULT + 10, acquire_thread_func, &lock);
 // msg ("Main thread should have priority %d. Actual priority: %d,. Origin_priority:%d ", 41, thread_get_priority(), thread_current()->origin_priority);
