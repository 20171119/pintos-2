...
//      printf("$$$$$$ process n : %d   child_pid = %d \n",n,child_pid);
      /* If maximum depth is reached, return result. */
      if (child_pid == -1)
      {
//        printf("**when n = %d  return start\n",n );
        return n;
      }

      /* Else wait for child to report how deeply it was able to recurse. */
      int reached_depth = wait (child_pid);
 //     printf("&& n = %d wait (child_pid) = wait (%d) \n",n,child_pid);
 //     printf("&& n = %d // reached_depth = %d \n", n, reached_depth);
      if (reached_depth == -1)
      {
 //       printf("before fail n is %d\n", n);
